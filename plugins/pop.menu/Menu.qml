...
Item {
  ...
  // Drives the open/close fade. Bound to `opened` and animated here so the
  // card background tint and content cross-fade while the border (on the
  // card) stays at full thickness.
  property real uiOpacity: root.opened ? 1 : 0
  ...
  // Smooth, subtle hairline border that reads as a defined surface without
  // fighting the glass background. Uses a low-alpha neon tint so it blends
  // with both light and dark themes.
  property var borderSpec: Border.flat(Color.menu.border.a > 0 ? Color.menu.border : "#efae64", 2)
  ...
  // Neon scheme to match the workspace widget: hovered rows glow a dimmed,
  // blacked green with lower contrast, idle rows use a beige at higher alphas
  // for readability (0.7 for text/icons, 0.4 for borders).
  readonly property color neonGreen: "#b3b1ab"
  readonly property color neonOrange: "#e8e8e6"
  readonly property color hoveredTextColor: "#edebe4"
  readonly property color hoveredBackgroundColor: Util.alpha("#efae64", 0.1)
  readonly property color hoveredBorderColor: Color.menu.border
  readonly property color idleTextColor: Util.alpha(root.neonOrange, 0.7)
  readonly property color idleBorderColor: Util.alpha(root.neonOrange, 0.2)
  readonly property color idleBackgroundColor: root.background
  readonly property var hoveredBorderSpec: Border.flat(root.hoveredBorderColor, 2)
  // Keep a 1px transparent border at rest so the cursor border fades in
  // without nudging the row's content insets.
  readonly property var idleBorderSpec: Border.flat("transparent", 1)
  readonly property real rowReservedBorderLeft: Math.max(Border.left(hoveredBorderSpec), Border.left(idleBorderSpec))
  readonly property real rowReservedBorderRight: Math.max(Border.right(hoveredBorderSpec), Border.right(idleBorderSpec))
  readonly property int cornerRadius: 0
  property int contentMargin: Style.spacing.panelPadding
  property int headerHeight: Math.max(Style.space(34), Style.font.title + Style.spacing.controlPaddingY * 2)
  property int contentSpacing: Style.spacing.md
  property int baseRowHeight: Math.max(Style.space(40), Style.font.body + Style.spacing.rowPaddingX * 2)
  ...
  PanelWindow {
    id: panel
    visible: (root.opened && root.rowsLoaded) || root.uiOpacity > 0.01
    ...
    Rectangle {
      anchors.fill: parent
      color: root.scrim
      // Fade the backdrop in sync with the card content.
    ...
    BorderSurface {
      ...
      // Solid, fully opaque menu background matching the top bar / popups.
      color: "#34332f"
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              radius: root.cornerRadius
              color: row.hasCursor ? root.hoveredBackgroundColor : root.idleBackgroundColor
              borderSpec: row.hasCursor ? root.hoveredBorderSpec : root.idleBorderSpec

              Behavior on color {
                enabled: root.uiOpacity > 0.99
              ...
              Text {
                ...
                text: row.icon
                color: row.hasCursor ? root.hoveredTextColor : root.idleTextColor
              ...
              Row {
                ...
                Text {
                  ...
                  color: row.hasCursor ? root.hoveredTextColor : root.idleTextColor
                  opacity: (row.kind === "menu" || row.kind === "link") ? (row.hasCursor ? 1.0 : 0.7) : 0
                  font.family: root.fontFamily
                  font.pixelSize: Style.font.heading
